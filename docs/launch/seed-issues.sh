...
"Add a \`CleanupTarget\` for Docker Desktop's build/image cache metadata under \`~/Library/Containers/com.docker.docker\` is protected (Containers is blocklisted), but \`~/.docker\` scratch data such as \`~/.docker/buildx\` cache dirs is fair game. Investigate what is safely regenerable, propose a root + \`min_age_days\`, and add it to \`default_targets()\` in \`maccleaner/infrastructure/macos_targets.py\`. See CONTRIBUTING.md for the safety rules."

new_target_issue "Target: Gradle cache (~/.gradle/caches)" \
"Add a \`CleanupTarget\` for \`~/.gradle/caches\` — downloaded dependencies and build caches that Gradle re-fetches on demand. Suggested: \`Risk.SAFE\`, \`min_age_days=30\`. Add to \`default_targets()\` in \`maccleaner/infrastructure/macos_targets.py\` and verify with \`python3 -m maccleaner scan --only gradle-cache -v\`."
...
"Small PySide6 task in \`maccleaner/presentation/gui/main_window.py\`: add Select all / Select none buttons above the tree so users can start from zero and opt items in. No changes outside the presentation layer."

new_target_issue "GUI: remember window size and column widths between launches" \
"Use \`QSettings\` in \`maccleaner/presentation/gui/main_window.py\` to persist window geometry and tree column widths. Presentation-layer only."

new_target_issue "CLI: --json output for scan reports" \
"Add \`maccleaner scan --json\` printing the ScanReport as JSON (path, target_id, size_bytes, age_days). Useful for scripting and for future tooling. Presentation-layer change in \`maccleaner/presentation/cli.py\` only."
